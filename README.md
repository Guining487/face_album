...
- ✏️ 给人物命名：点卡片上的“命名”按钮，起个名字方便认人
- 📂 导出相册到硬盘：按人物名字建子文件夹，可“复制”或建“快捷方式”
- 🖼️ 点击照片用系统默认程序打开原图
- ⚡ 可选 GPU 加速（有 NVIDIA 显卡时自动启用，否则回退 CPU）
- 🎨 毛玻璃界面：VSCode 星空背景 + 圆角窗口 + 半透明磨砂面板，卡片/按钮/名言带丝滑动画
- 顶部 macOS 风格三键（红/黄/绿），可拖动窗口、双击标题栏最大化
...
5. （可选）点卡片上的「✏️ 命名」给人物起名字
6. （可选）选「复制」或「快捷方式」，点「导出相册 ▶」
   选择输出文件夹后，程序会按人物名字建子文件夹并写入照片（未分类不会导出）
...
| 复制 / 快捷方式 | 复制 | 复制=原图保留，占双份空间；快捷方式=只建指向原图的 .lnk，几乎不占空间，同一照片有多张脸时可同时归到多个人物 |

## 目录结构

```
face_album/
├── face_album_qt.py    # 主程序（PySide6 GUI + 聚类逻辑）
├── run_gui.bat         # Windows 一键启动脚本
├── face_album.spec     # PyInstaller 打包配置
...
- 界面：PySide6 (Qt6)，无边框 + 透明背景实现圆角毛玻璃，后台线程 + 消息队列保证界面不卡死
